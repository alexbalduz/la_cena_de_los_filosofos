...
class filosofo(threading.Thread):
    ...
    def __init__(self, root):
        ...
        self.root.imprimir("FILOSOFO {0} - PENSANDO".format(self.id))
...
def main():
    lista=[]
    for i in range(N):
        lista.append(filosofo(frame1)) #AGREGA UN FILOSOFO A LA LISTA

    for f in lista:
        print(f.start()) #ES EQUIVALENTE A RUN()
    root.run()
